...
def run(env_name: str = "/Users/yanzeyang/Desktop/Group5RL-eg2140/Group5RL-eg2140/l2rpn_case14_storage_/l2rpn_case14_storage_train", agent:Literal['DDPG','TD3']="DDPG",
        n_active:int=500000, replay_size:int=100000, rho_threshold:float=0.6, stage:Literal["TRAIN","VALIDATE","TEST"]="TRAIN", 
        batch_size:int=1024, seed:int=0, verbose:bool=False) -> Tuple[float]:
    """
    ...
    ACT_LIMIT = 2 # Is it suits when we set 3?,can we reach the limit?
    ...
    agent: DDPG | TD3 = agent_class(agent_hparams(
        ...
        act_dim=ACT_DIM,
        act_limit=ACT_LIMIT,
        pi_hidden_sizes=[512, 256, 128],
        q_hidden_sizes=[1024, 512],
    ...
    #print(f"Max episode duration: {env.max_episode_duration()} steps")
    ...
    while total_steps < n_active:
        ...
        while not done:
            ...
            if np.all(env.tracker.state.rho < 0.7):
                reward += +10

            if np.all(env.tracker.state.rho < 0.6):
                reward += +10
            
            if np.all(env.tracker.state.rho < 0.5):
                reward += +10
            ...
            if np.any(env.tracker.state.rho >= 0.9):
                reward += -20
                print("  High line loading detected (rho ≥ 1). Penalty applied.")


            if np.any(env.tracker.state.rho >= 0.8):
                reward += -20
                print("  High line loading detected (rho ≥ 0.97). Penalty applied.")


            if np.any(env.tracker.state.rho >= 0.7):
                reward += -20
                print("  High line loading detected (rho ≥ 0.8). Penalty applied.")
